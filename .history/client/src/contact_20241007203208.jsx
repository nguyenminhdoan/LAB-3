...
import '../src/contact.css';
import { useState } from 'react';
import { create } from '../contact/api-contact'; // Adjust path accordingly

export default function Contact({ credentials }) {
    const [formData, setFormData] = useState({
        myFName: '',
        myLName: '',
        myEmail: '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await create({ contactId: 'new' }, credentials, JSON.stringify(contact));

            if (response && response._id) {
                console.log('Contact saved:', response);
                // Optionally, reset form or show a success message here
                setFormData({ myFName: '', myLName: '', myEmail: '' });
            } else {
